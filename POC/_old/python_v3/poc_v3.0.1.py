...
stocks = ['WHITF','TAL','CEIX','HKD','APE','MSFT','AFRM','COIN','TWLO','WEGE3.SA','WEGE3.SA_0601']
START_FOUNDS = 100_000
founds_start = START_FOUNDS
for stock in stocks:

#### leitura de arquivo
    file = open(f'../js/stocks/stocks_data/{stock}.json')
...
#### bloqueios
    ...
    #### 1° stop-less, passou o bloqueio mas verifica se nao esta caindo muito Rapido
        ...
        if len(curr_founds_bellow_1000) > 15:
            ...
            if rapid_falling(curr_founds_bellow_1000) and curr_founds >= founds_start:
...
#### breaks 
    #### ganho suficiente 
        # 0.6 de lucro
        if not holding and curr_founds >= founds_start * 1.006:
        ...
        # 0.6 de lucro
        if not holding and curr_founds >= founds_start * 1.003:
        ...
        # 0.5 de lucro
        elif holding and curr_founds >= founds_start * 1.005: #or (index == 29 and stock == 'COIN'):
        ...
        # 0.5 de lucro
        elif holding and curr_founds >= founds_start * 1.002: #or (index == 29 and stock == 'COIN'):
    ...
    #### quase acabando dia
        # passou 75% do tempo
        elif stops(holding, index, 0.75, break_point, curr_fluct, holded, curr_founds, founds_start * 1.04):
        ...
        # passou 85% do tempo
        elif stops(holding, index, 0.85, break_point, curr_fluct, holded, curr_founds, founds_start * 1.03):
...
#### 2° stop-less
    ### change de stop-less
        # caso tenha algo comprado
            ...
            curr_possivle_win = curr_founds - founds_start
            last_win = curr_founds - founds_start 
            # compara se o lucro atual com anterior
            if curr_founds > founds_start and curr_possivle_win <= last_win * 0.9:
                founds = curr_founds
                curr_founds = founds
                holding = False
        ...
        elif not holding and founds <= founds_start * 0.998:
            buys_and_sells.append(('PANIC!',index))
            print(buys_and_sells)
            break;
...
#### imprime resultados
    ...
    if founds > START_FOUNDS:
        print(f'{founds} lucro, {round(founds - START_FOUNDS, 4)}')
    if founds == START_FOUNDS:
        print('deu na mesma')
    if founds < START_FOUNDS and founds > 0:
        print(f'{founds} prejuizo, perdemos {round(founds - START_FOUNDS, 4)}')
    if founds <= 0:
